package com.kjq.common.utils.statusBar;
...
public class StatusBarUtil {
     ...
     * @param colorId 颜色
     */
    public static void setStatusBarColor(@ColorRes int colorId) {
        Activity activity = AppManager.getActivityStack().lastElement();
        int sI_color = ContextCompat.getColor(activity,colorId);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            Window window = activity.getWindow();
            window.setStatusBarColor(sI_color);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            //使用SystemBarTintManager,需要先将状态栏设置为透明
            setTranslucentStatus(activity);
            SystemBarTintManager systemBarTintManager = new SystemBarTintManager(activity);
            systemBarTintManager.setStatusBarTintEnabled(true);//显示状态栏
            systemBarTintManager.setStatusBarTintColor(sI_color);//设置状态栏颜色
        }
    }
     ...
     * @param activity 当前显示的
     * @param isFullscreen 是否显示标题
     */
    public static void setRootViewFullscreen(Activity activity, boolean isFullscreen) {
     ...
     * 代码实现android:fitsSystemWindows
     * @param activity 当前显示的
     * @param fitSystemWindows 是否
     */
    public static void setRootViewFitsSystemWindows(Activity activity, boolean fitSystemWindows,boolean translucentStatus) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
