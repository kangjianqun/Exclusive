package com.kjq.common.ui.designs.refreshLayout;
...
import com.kjq.common.ui.designs.refreshLayout.api.DefaultRefreshFooterCreator;
...
import com.kjq.common.ui.designs.refreshLayout.api.RefreshContent;
import com.kjq.common.ui.designs.refreshLayout.api.RefreshFooter;
import com.kjq.common.ui.designs.refreshLayout.api.RefreshHeader;
import com.kjq.common.ui.designs.refreshLayout.api.RefreshInternal;
import com.kjq.common.ui.designs.refreshLayout.api.RefreshKernel;
import com.kjq.common.ui.designs.refreshLayout.api.RefreshLayout;
import com.kjq.common.ui.designs.refreshLayout.api.ScrollBoundaryDecider;
import com.kjq.common.ui.designs.refreshLayout.constant.DimensionStatus;
...
import com.kjq.common.ui.designs.refreshLayout.footer.BallPulseFooter;
import com.kjq.common.ui.designs.refreshLayout.header.BezierRadarHeader;
import com.kjq.common.ui.designs.refreshLayout.impl.RefreshContentWrapper;
import com.kjq.common.ui.designs.refreshLayout.impl.RefreshFooterWrapper;
import com.kjq.common.ui.designs.refreshLayout.impl.RefreshHeaderWrapper;
import com.kjq.common.ui.designs.refreshLayout.listener.OnLoadMoreListener;
import com.kjq.common.ui.designs.refreshLayout.listener.OnMultiPurposeListener;
import com.kjq.common.ui.designs.refreshLayout.listener.OnRefreshListener;
import com.kjq.common.ui.designs.refreshLayout.listener.OnRefreshLoadMoreListener;
import com.kjq.common.ui.designs.refreshLayout.listener.OnStateChangedListener;
import com.kjq.common.ui.designs.refreshLayout.util.DensityUtil;
import com.kjq.common.ui.designs.refreshLayout.util.ViscousFluidInterpolator;
...
import static com.kjq.common.ui.designs.refreshLayout.util.DensityUtil.dp2px;
import static com.kjq.common.ui.designs.refreshLayout.util.SmartUtil.fling;
import static com.kjq.common.ui.designs.refreshLayout.util.SmartUtil.isContentView;
...
//        mNestedChild.setNestedScrollingEnabled(true);//默认开启嵌套滚动
        ...
        mDragRate = ta.getFloat(R.styleable.SmartRefreshLayout_commonDragRate, mDragRate);
        mHeaderMaxDragRate = ta.getFloat(R.styleable.SmartRefreshLayout_commonHeaderMaxDragRate, mHeaderMaxDragRate);
        mFooterMaxDragRate = ta.getFloat(R.styleable.SmartRefreshLayout_commonFooterMaxDragRate, mFooterMaxDragRate);
        mHeaderTriggerRate = ta.getFloat(R.styleable.SmartRefreshLayout_commonHeaderTriggerRate, mHeaderTriggerRate);
        mFooterTriggerRate = ta.getFloat(R.styleable.SmartRefreshLayout_commonFooterTriggerRate, mFooterTriggerRate);
        mEnableRefresh = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableRefresh, mEnableRefresh);
        mReboundDuration = ta.getInt(R.styleable.SmartRefreshLayout_commonReboundDuration, mReboundDuration);
        mEnableLoadMore = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableLoadMore, mEnableLoadMore);
        mHeaderHeight = ta.getDimensionPixelOffset(R.styleable.SmartRefreshLayout_commonHeaderHeight, mHeaderHeight);
        mFooterHeight = ta.getDimensionPixelOffset(R.styleable.SmartRefreshLayout_commonFooterHeight, mFooterHeight);
        mHeaderInsetStart = ta.getDimensionPixelOffset(R.styleable.SmartRefreshLayout_commonHeaderInsetStart, mHeaderInsetStart);
        mFooterInsetStart = ta.getDimensionPixelOffset(R.styleable.SmartRefreshLayout_commonFooterInsetStart, mFooterInsetStart);
        mDisableContentWhenRefresh = ta.getBoolean(R.styleable.SmartRefreshLayout_commonDisableContentWhenRefresh, mDisableContentWhenRefresh);
        mDisableContentWhenLoading = ta.getBoolean(R.styleable.SmartRefreshLayout_commonDisableContentWhenLoading, mDisableContentWhenLoading);
        mEnableHeaderTranslationContent = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableHeaderTranslationContent, mEnableHeaderTranslationContent);
        mEnableFooterTranslationContent = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableFooterTranslationContent, mEnableFooterTranslationContent);
        mEnablePreviewInEditMode = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnablePreviewInEditMode, mEnablePreviewInEditMode);
        mEnableAutoLoadMore = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableAutoLoadMore, mEnableAutoLoadMore);
        mEnableOverScrollBounce = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableOverScrollBounce, mEnableOverScrollBounce);
        mEnablePureScrollMode = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnablePureScrollMode, mEnablePureScrollMode);
        mEnableScrollContentWhenLoaded = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableScrollContentWhenLoaded, mEnableScrollContentWhenLoaded);
        mEnableScrollContentWhenRefreshed = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableScrollContentWhenRefreshed, mEnableScrollContentWhenRefreshed);
        mEnableLoadMoreWhenContentNotFull = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableLoadMoreWhenContentNotFull, mEnableLoadMoreWhenContentNotFull);
        mEnableFooterFollowWhenNoMoreData = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableFooterFollowWhenLoadFinished, mEnableFooterFollowWhenNoMoreData);
        mEnableFooterFollowWhenNoMoreData = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableFooterFollowWhenNoMoreData, mEnableFooterFollowWhenNoMoreData);
        mEnableClipHeaderWhenFixedBehind = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableClipHeaderWhenFixedBehind, mEnableClipHeaderWhenFixedBehind);
        mEnableClipFooterWhenFixedBehind = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableClipFooterWhenFixedBehind, mEnableClipFooterWhenFixedBehind);
        mEnableOverScrollDrag = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableOverScrollDrag, mEnableOverScrollDrag);
        mFixedHeaderViewId = ta.getResourceId(R.styleable.SmartRefreshLayout_commonFixedHeaderViewId, mFixedHeaderViewId);
        mFixedFooterViewId = ta.getResourceId(R.styleable.SmartRefreshLayout_commonFixedFooterViewId, mFixedFooterViewId);
        mHeaderTranslationViewId = ta.getResourceId(R.styleable.SmartRefreshLayout_commonHeaderTranslationViewId, mHeaderTranslationViewId);
        mFooterTranslationViewId = ta.getResourceId(R.styleable.SmartRefreshLayout_commonFooterTranslationViewId, mFooterTranslationViewId);
        mEnableNestedScrolling = ta.getBoolean(R.styleable.SmartRefreshLayout_commonEnableNestedScrolling, mEnableNestedScrolling);
//        mEnableNestedScrollingOnly = ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnableNestedScrollingOnly, mEnableNestedScrollingOnly);
        ...
        mManualLoadMore = mManualLoadMore || ta.hasValue(R.styleable.SmartRefreshLayout_commonEnableLoadMore);
        mManualHeaderTranslationContent = mManualHeaderTranslationContent || ta.hasValue(R.styleable.SmartRefreshLayout_commonEnableHeaderTranslationContent);
        mManualFooterTranslationContent = mManualFooterTranslationContent || ta.hasValue(R.styleable.SmartRefreshLayout_commonEnableFooterTranslationContent);
//        mManualNestedScrolling = mManualNestedScrolling || ta.hasValue(R.styleable.SmartRefreshLayout_srlEnableNestedScrolling);
        mHeaderHeightStatus = ta.hasValue(R.styleable.SmartRefreshLayout_commonHeaderHeight) ? DimensionStatus.XmlLayoutUnNotify : mHeaderHeightStatus;
        mFooterHeightStatus = ta.hasValue(R.styleable.SmartRefreshLayout_commonFooterHeight) ? DimensionStatus.XmlLayoutUnNotify : mFooterHeightStatus;

        int accentColor = ta.getColor(R.styleable.SmartRefreshLayout_commonAccentColor, 0);
        int primaryColor = ta.getColor(R.styleable.SmartRefreshLayout_commonPrimaryColor, 0);
        if (primaryColor != 0) {
            if (accentColor != 0) {
...
//            }
            ...
            if (mRefreshContent == null) {
                ...
                errorView.setText(R.string.common_content_empty);
...
//            }
    ...
    protected void onMeasure(final int widthMeasureSpec,final int heightMeasureSpec) {
        ...
        for (int i = 0, len = super.getChildCount(); i < len; i++) {
            ...
            if (child.getVisibility() == GONE || child.getTag(R.string.common_component_falsify) == child) {
                continue;
            }
...
//    }
    ...
    public LayoutParams generateLayoutParams(AttributeSet attrs) {
        final View thisView = this;
        return new SmartRefreshLayout_Layout(thisView.getContext(), attrs);
    }

    public static class SmartRefreshLayout_Layout extends MarginLayoutParams {

        public SmartRefreshLayout_Layout(Context context, AttributeSet attrs) {
            super(context, attrs);
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SmartRefreshLayout_Layout);
            backgroundColor = ta.getColor(R.styleable.SmartRefreshLayout_Layout_commonBackgroundColor, backgroundColor);
            if (ta.hasValue(R.styleable.SmartRefreshLayout_Layout_commonSpinnerStyle)) {
                spinnerStyle = SpinnerStyle.values()[ta.getInt(R.styleable.SmartRefreshLayout_Layout_commonSpinnerStyle, SpinnerStyle.Translate.ordinal())];
            }
            ta.recycle();
        }

        public SmartRefreshLayout_Layout(int width, int height) {
            super(width, height);
        }
...
//    }
    ...
    public RefreshLayout setRefreshFooter(@NonNull RefreshFooter footer, int width, int height) {
        ...
        if (mRefreshFooter.getSpinnerStyle() == SpinnerStyle.FixedBehind) {
            super.addView(mRefreshFooter.getView(), 0, new SmartRefreshLayout_Layout(width, height));
        } else {
    ...
    public RefreshLayout setRefreshContent(@NonNull View content, int width, int height) {
        ...
        super.addView(content, 0, new SmartRefreshLayout_Layout(width, height));
        if (mRefreshHeader != null && mRefreshHeader.getSpinnerStyle() == SpinnerStyle.FixedBehind) {
