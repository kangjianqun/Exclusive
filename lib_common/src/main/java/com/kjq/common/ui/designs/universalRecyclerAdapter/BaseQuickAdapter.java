 ...
 */
package com.kjq.common.ui.designs.universalRecyclerAdapter;
...
import com.kjq.common.ui.designs.universalRecyclerAdapter.animAction.AlphaInAnimation;
import com.kjq.common.ui.designs.universalRecyclerAdapter.animAction.BaseAnimation;
import com.kjq.common.ui.designs.universalRecyclerAdapter.animAction.ScaleInAnimation;
import com.kjq.common.ui.designs.universalRecyclerAdapter.animAction.SlideInBottomAnimation;
import com.kjq.common.ui.designs.universalRecyclerAdapter.animAction.SlideInLeftAnimation;
import com.kjq.common.ui.designs.universalRecyclerAdapter.animAction.SlideInRightAnimation;
import com.kjq.common.ui.designs.universalRecyclerAdapter.loadMore.LoadMoreView;
import com.kjq.common.ui.designs.universalRecyclerAdapter.loadMore.SimpleLoadMoreView;
import com.kjq.common.ui.designs.universalRecyclerAdapter.model.IExpandable;
...
public abstract class BaseQuickAdapter<Entity, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
    ...
    public void setShouldScroll(boolean shouldScroll){
        this.mShouldScroll = shouldScroll;
    }
    ...
    public void smoothMoveToPosition(final int position) {
        ...
        if (position < firstItem) {
            ...
            if (movePosition >= 0 && movePosition < mRecyclerView.getChildCount()) {
                ...
                LinearLayoutManager sLinearLayoutManager = (LinearLayoutManager) mRecyclerView.getLayoutManager();

                assert sLinearLayoutManager != null;
                if (sLinearLayoutManager.getOrientation() == RecyclerView.VERTICAL){
                    int sTop = mRecyclerView.getChildAt(movePosition).getTop();
                    mRecyclerView.smoothScrollBy(0, sTop);
                }else if (sLinearLayoutManager.getOrientation() == RecyclerView.HORIZONTAL){
                    int sLeft = mRecyclerView.getChildAt(movePosition).getLeft();
                    mRecyclerView.smoothScrollBy(sLeft, 0);
                }
            }
        } else {
     ...
     */
    public BaseQuickAdapter(@LayoutRes int layoutResId, @Nullable List<Entity> data) {
        this.mData = data == null ? new ArrayList<Entity>() : data;
        if (layoutResId != 0) {
            this.mLayoutResId = layoutResId;
        }
    }

    public BaseQuickAdapter(@Nullable List<Entity> data) {
        this(0, data);
    }
     ...
     * @param data
     */
    public void setNewData(@Nullable List<Entity> data) {
        this.mData = data == null ? new ArrayList<Entity>() : data;
     ...
     */
    @Deprecated
    public void add(@IntRange(from = 0) int position, @NonNull Entity item) {
     ...
     * @param position
     */
    public void addData(@IntRange(from = 0) int position, @NonNull Entity data) {
     ...
     * add one new data
     */
    public void addData(@NonNull Entity data) {
        mData.add(data);
     ...
     * change data
     */
    public void setData(@IntRange(from = 0) int index, @NonNull Entity data) {
     ...
     */
    public void addData(@IntRange(from = 0) int position, @NonNull Collection<? extends Entity> newData) {
     ...
     * @param newData the new data collection
     */
    public void addData(@NonNull Collection<? extends Entity> newData) {
     ...
     */
    public void replaceData(@NonNull Collection<? extends Entity> data) {
        // 不是同一个引用才清空列表
        if (data != mData) {
            mData.clear();
            mData.addAll(data);
     ...
     */
    @Nullable
    public Entity getItem(@IntRange(from = 0) int position) {
    ...
    public void onBindViewHolder(K holder, int position) {
        ...
        // 添加fetch逻辑，几乎像加载更多，但更简单。
        autoUpFetch(position);
        //Do not move position, need to change before LoadMoreView binding
        // 不要移动位置，需要在LoadMoreView绑定之前进行更改
    ...
    private MultiTypeDelegate<Entity> mMultiTypeDelegate;

    public void setMultiTypeDelegate(MultiTypeDelegate<Entity> multiTypeDelegate) {
        mMultiTypeDelegate = multiTypeDelegate;
    }

    public MultiTypeDelegate<Entity> getMultiTypeDelegate() {
     ...
     */
    protected abstract void convert(K helper, Entity item);
...
//            subItemCount += list.size();
    ...
    private int recursiveCollapse(@IntRange(from = 0) int position) {
        ...
        int subItemCount = 0;
        if (expandable.isExpanded()) {
            List<Entity> subItems = expandable.getSubItems();
            if (null == subItems) return 0;

            for (int i = subItems.size() - 1; i >= 0; i--) {
                Entity subItem = subItems.get(i);
    ...
    private int getItemPosition(Entity item) {
    ...
    public boolean isExpandable(Entity item) {
     ...
     */
    public int getParentPosition(@NonNull Entity item) {
